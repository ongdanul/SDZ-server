...
public interface VerificationApiDocs {
    ...
    ResponseEntity<Map<String, Boolean>> checkEmailExists(@RequestBody Map<String, String> requestBody);
    ...
    ResponseEntity<Map<String, Boolean>> checkAccountLimit(@RequestBody Map<String, String> requestBody);

    @Operation(summary = "일반 회원 비밀번호 찾기시 이메일 존재 여부 확인", description = "일반 회원 비밀번호 찾기시 입력한 이메일과 이름으로 조회하여 일치하는 이메일이 존재하는지 확인하는 API입니다.")
    ResponseEntity<Map<String, Boolean>> validateEmailExists(@RequestBody Map<String, String> requestBody);
